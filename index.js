...
function get20s(array) {
  ...
  for(let i = 0; i < array.length; i++) {
    let yearsArr = [];
    yearsArr = array[i].years.split(' - ');
    const yearStart = yearsArr[0];
    const yearEnd = yearsArr[1];
    if(yearStart >= 1900 && yearEnd <= 2000) {
...
function addArtist(array) {
  const addThisObject = { 
    ...
    bio: 'I love being a father and the way these tests are setup is awesome'
  }  
  array.push(addThisObject);
  return array
}
